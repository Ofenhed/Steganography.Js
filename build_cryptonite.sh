...
FLAGS="-s EXPORTED_FUNCTIONS=$EXPORTED_FUNCTIONS -O2 -msse -msse2 -msse3 -s ASSERTIONS=1 -s TOTAL_MEMORY=1006632960 -DWITH_DEEPSEQ_SUPPORT -DWITH_ASSERT_ALIGNMENT" #-maes -mpclmul -s SIMD=1
CFLAGS="$FLAGS -DWITH_ASSERT_ALIGNMENT -D__NOT__SOMETHING__ARCH_IS_LITTLE_ENDIAN -D__NOT__SOMETHING__ARCH_X86 -DSUPPORT_SSE -DED25519_NO_INLINE_ASM -DNO_INLINE_ASM" #-DWITH_PCLMUL" # -DWITH_AESNI"
LDFLAGS="$FLAGS -s STACK_OVERFLOW_CHECK=1 -s NO_EXIT_RUNTIME=1 -s BINARYEN_METHOD='native-wasm,asmjs' -s BINARYEN=1 -s MODULARIZE=1 -s OUTLINING_LIMIT=1000"
